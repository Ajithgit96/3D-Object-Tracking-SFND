...
void loadLidarFromFile(vector<LidarPoint> &lidarPoints, string filename)
{
    ...
    unsigned long num = 1000000;  //around 100K points present in single LiDAR frame. Here num/4 = 250K. Below space is sufficient to store 250K 3D LiDAR points.
    float *data = (float*)malloc(num*sizeof(float)); // Malloc (and also new, delete operators) function helps in accessing heap memory
    ...
    // load point cloud
    FILE *stream;
    stream = fopen (filename.c_str(),"rb");  //opening the file containing 3D LiDAR points
    num = fread(data,sizeof(float),num,stream)/4; // calculates the number of 3D LiDAR points stored in file
